...
$colname_rs_p = "-1";
if (isset($_GET['bId'])) {
  $colname_rs_p = $_GET['bId'];
}

$query_rs_p = sprintf("SELECT * FROM albumpicture WHERE bId = %s", GetSQLValueString($gra2017, $colname_rs_p, "int"));
$rs_p = mysqli_query($gra2017,$query_rs_p);
$row_rs_p = mysqli_fetch_assoc($rs_p);
$totalRows_rs_p = mysqli_num_rows($rs_p);

if ((isset($_GET['bId'])) && ($_GET['bId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM albumpicture WHERE bId=%s",
                       GetSQLValueString($gra2017, $_GET['bId'], "int"));

  
  $Result1 = mysqli_query($gra2017, $deleteSQL);

  $deleteGoTo = "albumpicture.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
    $deleteGoTo .= "aId=" . $row_rs_p['bLid'];
  }
  @unlink('./Picture/albumimg/' . $row_rs_p['bImg']);
