...
$colname_rs_p = "-1";
if (isset($_GET['pId'])) {
  $colname_rs_p = $_GET['pId'];
}

$query_rs_p = sprintf("SELECT * FROM photo WHERE pId = %s", GetSQLValueString($gra2017, $colname_rs_p, "int"));
$rs_p = mysqli_query($gra2017, $query_rs_p);
$row_rs_p = mysqli_fetch_assoc($rs_p);
$totalRows_rs_p = mysqli_num_rows($rs_p);

if ((isset($_GET['pId'])) && ($_GET['pId'] != "")) {
  $deleteSQL = sprintf("DELETE FROM photo WHERE pId=%s",
                       GetSQLValueString($gra2017, $_GET['pId'], "int"));

  
  $Result1 = mysqli_query($gra2017, $deleteSQL);

  $deleteGoTo = "photo.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
    $deleteGoTo .= "aId=" . $row_rs_p['pLalbum'] . "&pageNum_rs_photo=" . $_GET['page'];
  }
  @unlink('./Picture/thumb/' . $row_rs_p['pFilename']);
  @unlink('./Picture/' . $row_rs_p['pFilename']);
  header(sprintf("Location: %s", $deleteGoTo));
}



mysqli_free_result($rs_p);
?>
